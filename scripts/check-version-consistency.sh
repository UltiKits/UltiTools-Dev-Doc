...
cat <<'NOTE'
  ...
  但旧 pin 不等于「向后兼容已被证明」。有两种形状能打破它：

    · 框架的 MINOR 版本可以移除 API —— 还在用已移除类型的模块会 NoClassDefFoundError。
      把 pin 临时调高、跑一次构建、看什么编译不过，是一次有价值的排查；这一种的防线
      是跟进废弃通告。
    · 框架也可能不移除任何东西，只改掉一个公开方法的描述符（6.1.1 → 6.2.1 就把
      getContext() 的返回类型改了）—— 已编译的 JAR 拿到 NoSuchMethodError，而源码
      是兼容的。这一种没有通告可跟：版本号策略排期的是有意为之的移除，无意的描述符
      变更不在排期里，PATCH 一样可能中。只能靠框架版本一变就重新编译并重新发布。
