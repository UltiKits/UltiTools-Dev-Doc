...
::: warning @UltiToolsModule 上的 eventListener 与 cmdExecutor 从不被读取
...
改为直接在你的类上标注 `@EnableAutoRegister(eventListener = false)` 或 `cmdExecutor = false`：`PluginManager` 是按直接查找读取这个注解类型的，标在它自己身上的值才会生效。
...
`flag = false` 时，`registerBukkit` 把命令注册交给 `CommandManager.registerAll(UltiToolsPlugin, String)`，它把每个扫描到的类直接强转成已废弃的 `AbstractCommandExecutor`，现已标 `@Deprecated(since = "6.2.5", forRemoval = true)`，因此按推荐写法继承 `BaseCommandExecutor` 的命令类会抛出未捕获的 `ClassCastException`。
