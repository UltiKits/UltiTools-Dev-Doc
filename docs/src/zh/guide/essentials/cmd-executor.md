...
::: danger 方法体涉及世界、实体、方块、区块或调度器状态时不得标注 @RunAsync
异步在这里只用于纯 CPU 或 I/O 型的工作。异步方法体如果需要回到 Bukkit 状态，必须通过 `Bukkit.getScheduler().runTask(...)`，就像上面示例那样。
:::

未标注 `@RunAsync` 的命令方法体并不会在调用线程上直接执行：框架本身会通过 `runTask()` 把它推迟一个 tick 执行。因此把不需要异步的方法体上的 `@RunAsync` 去掉，并不会让命令阻塞服务器，只是恢复了其他命令同样使用的这套延迟一个 tick 的同步调度。违反上面这条规则时看到的不是变慢，而是崩溃：Paper 的异步操作检查会拒绝第一次对世界、实体、方块或区块的访问，命令在方法体执行中途终止。真正需要异步执行的工作应改用 `@AsyncCommand`，它在这个边界上额外提供处理中提示和超时。
