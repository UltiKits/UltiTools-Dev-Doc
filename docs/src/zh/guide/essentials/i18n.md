...
::: warning 实际使用的语言由 UltiTools 主插件配置决定
实际使用的语言取自 UltiTools 主插件配置中的 `language` 键，由 `getLanguageCode()` 读取，而 `@I18n` 与重写的 `supported()` 是声明性元数据，运行时没有读取方。
把每个语言文件命名为 `lang/<语言码>.json`，确保其中一个与该键的值一致：没有匹配的文件时，模块会退回空字典，`i18n("some.key")` 静默返回你传入的键名。
给 `supported()` 补上运行时作用或将其移除的决定跟踪于 [issue #315](https://github.com/UltiKits/UltiTools-Reborn/issues/315)。
:::
