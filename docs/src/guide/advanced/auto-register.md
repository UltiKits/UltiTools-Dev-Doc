...
::: warning eventListener and cmdExecutor on @UltiToolsModule are never read
...
Put `@EnableAutoRegister(eventListener = false)` or `cmdExecutor = false` directly on your class instead: `PluginManager` reads that annotation type by direct lookup, so a value declared on it does apply.
...
With `flag = false`, `registerBukkit` sends command registration to `CommandManager.registerAll(UltiToolsPlugin, String)`, which casts every scanned class straight to the retired `AbstractCommandExecutor` and is now `@Deprecated(since = "6.2.5", forRemoval = true)`, so a command class written against the recommended `BaseCommandExecutor` throws an uncaught `ClassCastException`.
