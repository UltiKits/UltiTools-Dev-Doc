...
`@ConditionalOnConfig` is read only in `ComponentScanner.shouldRegister`, which sits on the container scan path, while a plugin registered through `PluginManager.register(...)` reaches its command executors and listeners through a package scan that instantiates them reflectively, so the annotation is never consulted and the class is registered whatever the config says.
