...
Asynchrony here is reserved for pure-CPU or I/O work. The one Bukkit call an async body may make is scheduling its state-touching work back onto the main thread, either `BukkitRunnable#runTask(...)` as in the example above or `Bukkit.getScheduler().runTask(...)` directly; the annotation never grants safe access to those APIs by itself. A handler whose body consists only of such state access has no reason to carry the annotation at all.
:::
